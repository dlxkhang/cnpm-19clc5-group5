...
module.exports.getNewProductListByName = (key) => {
    return new Promise(function(resolve, reject) {
        key = '%' + key + '%'
        var query = "SELECT distinct p.PID as productId, c.CATEGORY_NAME as productCategory, p.PRODUCT_NAME as productName, w.UNIT_PRICE as productPrice, s.STORE_NAME as sellerName, s.SID, p.IMAGE_URL as imageURI, p.DESCRIPTION as productDescription, w.STOCK as stock FROM Product p JOIN Warehouse w ON(p.PID = w.PID) JOIN Seller s ON(w.SID = s.SID) JOIN Category c ON(c.CID = p.CATEGORY) WHERE w.stock > 0 AND p.PRODUCT_NAME LIKE ?"
...
module.exports.getNewProductListByCategory = (key) => {
    return new Promise(function(resolve, reject) {
        key = '%' + key + '%'
        var query = "SELECT p.PID as productId, c.CATEGORY_NAME as productCategory, p.PRODUCT_NAME as productName, w.UNIT_PRICE as productPrice, s.STORE_NAME as sellerName, s.SID, p.IMAGE_URL as imageURI, p.DESCRIPTION as productDescription, w.STOCK as stock FROM Product p JOIN Warehouse w ON(p.PID = w.PID) JOIN Seller s ON(w.SID = s.SID) JOIN Category c ON(c.CID = p.CATEGORY) WHERE w.stock > 0 AND c.CATEGORY_NAME LIKE ?"
        db.all(query, key, function(err, allRows) {
            if(err) {
                reject(err)
                return
