...
async function getOrderedProducts(OID, listSID) {
    return new Promise(async function(resolve, reject) {
        ...
        for(const SID of listSID) {
            // get ordered product
            var query = "SELECT distinct p.PID as productId, p.CATEGORY as productCategory, p.PRODUCT_NAME as productName, w.UNIT_PRICE as productPrice, s.STORE_NAME as sellerName, s.SID, p.IMAGE_URL as imageURI, p.DESCRIPTION as productDescription, w.STOCK as stock FROM Product p JOIN Warehouse w ON(p.PID = w.PID) JOIN OrderDetail od ON(od.PID = p.PID) JOIN Seller s ON(od.SID = s.SID) WHERE od.OID = ? AND w.SID = ? AND od.SID = ?"
